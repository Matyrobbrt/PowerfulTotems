...
public class WaterBreathingTotem extends Item {

	public WaterBreathingTotem() {
	...
	public static void damageItem(ItemStack stack) {
		...
		public ActionResult<ItemStack> use(World worldIn, PlayerEntity player, Hand hand) {
			...
			if (!player.hasEffect(Effects.WATER_BREATHING)) {
				player.addEffect(new EffectInstance(Effects.WATER_BREATHING, TotemConfig.water_breathing_totem_effect_duration.get()*20, 0, true, true));
				damageItem(stack);
			} else {
				if (worldIn.isClientSide) player.sendMessage(new StringTextComponent("\u00A7b" + "You already have the Water Breathing effect!"), player.getUUID());
			}
		...
		public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
			...
			StringTextComponent text = new StringTextComponent("\u00A7b" + "Right click to get " + "\u00A76" + "Water Breathing" + "\u00A7b" + " effect for "  + TotemConfig.water_breathing_totem_effect_duration.get() + " seconds.");
			tooltip.add(text);
