...
public class FireResistanceTotem extends Item {

	public FireResistanceTotem() {
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity player, Hand hand) {
		...
		if (!player.hasEffect(Effects.FIRE_RESISTANCE)) {
			player.addEffect(new EffectInstance(Effects.FIRE_RESISTANCE, TotemConfig.fire_resistance_effect_duration.get()*20, 0, true, true));
			damageItem(stack);
		} else {
			if (worldIn.isClientSide) player.sendMessage(new StringTextComponent("\u00A7b" + "You already have fire resistance!"), player.getUUID());
		}
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		StringTextComponent text = new StringTextComponent("\u00A7b" + "Right click to get " + "\u00A76" + "Fire Resistance" + "\u00A7b" + " effect for " + TotemConfig.fire_resistance_effect_duration.get() + " seconds.");
		tooltip.add(text);
