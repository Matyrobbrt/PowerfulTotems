...
public class SpeedTotem extends Item {
	...
	public ActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
		...
		if (player.hasEffect(Effects.MOVEMENT_SPEED) == true && player.getEffect(Effects.MOVEMENT_SPEED)
			...
			int level = player.getEffect(Effects.MOVEMENT_SPEED).getAmplifier() + 1;
			player.addEffect(new EffectInstance(Effects.MOVEMENT_SPEED,
					TotemConfig.speed_totem_effect_duration.get() * 20, level, true, true));
			damageItem(stack);
		} else if (player.hasEffect(Effects.MOVEMENT_SPEED) == false) {
			player.addEffect(new EffectInstance(Effects.MOVEMENT_SPEED,
					TotemConfig.speed_totem_effect_duration.get() * 20, 0, true, true));
			damageItem(stack);
		} else {
			if (world.isClientSide) player.sendMessage(new StringTextComponent("\u00A7b" + "You have reached the maximum level of Speed!"),
					player.getUUID());
		}
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		StringTextComponent text = new StringTextComponent("\u00A7b" + "Right click to get " + "\u00A76" + "Speed" + "\u00A7b" + " effect for " + TotemConfig.speed_totem_effect_duration.get() + " seconds.");
		tooltip.add(text);
