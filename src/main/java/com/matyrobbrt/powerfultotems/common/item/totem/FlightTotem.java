...
public class FlightTotem extends Item {
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity player, Hand hand) {
		...
		if (!player.hasEffect(PotionInit.FLIGHT.get())) {
			player.addEffect(new EffectInstance(PotionInit.FLIGHT.get(),
					TotemConfig.flight_totem_effect_duration.get() * 20, 0, true, true));
			damageItem(stack);
		} else {
			if (worldIn.isClientSide) player.sendMessage(new StringTextComponent("\u00A7d" + "You already can fly!"), player.getUUID());
		}

		//player.sendMessage(new StringTextComponent("The Fligth Totem is a Work In Progress!"), player.getUUID());
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip,
		...
		StringTextComponent text = new StringTextComponent("\u00A7b" + "Right click to get " + "\u00A7d" + "Flight"
				+ "\u00A7b" + " effect for " + TotemConfig.flight_totem_effect_duration.get() + " seconds.");
		tooltip.add(text);
